...
action offsets, because $\veco = \partial H / \partial \vecj$. For an
...
that used in \citep{Sanders13b}. The stream is constructed by running
...
The distribution of frequencies is three dimensional. A general
description in terms of a Gaussian distribution would have nine free
parameters. However, we can model it with fewer free parameters as
follows. First, following \citet{Eyre11a}, we approximate the
\emph{action} distribution as a Gaussian with standard deviations
\begin{equation}
...
variance to frequency space using the Hessian $(\partial^2 H /
\partial \vecj \partial \vecj)\large|_{\vecj^p}$\footnote{The Hessian
  $(\partial^2 H / \partial \vecj \partial \vecj)\large|_{\vecj^p}$
  can be computed using the method in \appendixname~\ref{sec:aa} by
  computing $\partial (\vecj,\veca) / \partial (\vecx,\vecv)$ and
  $\partial (\veco,\veca) / \partial (\vecx,\vecv)$ and forming
  $\partial \veco / \partial \vecj = \left(\partial (\vecj,\veca) /
  \partial (\vecx,\vecv)\right)^{-1}\,\left(\partial (\veco,\veca) /
  \partial (\vecx,\vecv)\right)$.}, which gives the variance matrix
...
\sigma_{\Omega,1}$, where $\sigma_{\Omega,1}$ is the square root of the
largest eigenvalue of $V_\veco$ and $\Delta \Omega^m$ is the mean
frequency offset. \figurename~\ref{fig:gd1_dohist} shows the
distribution frequency offsets in the direction of the median
frequency offset in the stream. The dashed line is a Gaussian fit to
this distribution, while the solid line is a fit of the form
...
\begin{figure}[t!]
  ...
  \caption{Mock tidal stream simulated as described in
    \sectionname~\ref{sec:sim} after $5.01\Gyr$ of evolution in
...
\begin{figure}[t!]
  ...
  \caption{Simulated tidal stream from \sectionname~\ref{sec:sim}
    after $5.01\Gyr$ of evolution in action--frequency--angle
...
\begin{figure}[t!]
  ...
  \caption{Simulated stream after $5.01\Gyr$ as a function of Galactic
...
\begin{figure}[t!]
 ...
 \includegraphics[width=0.48\textwidth,clip=]{gd1_evol_2Gya_xz_sim.ps}
  \caption{Mock stream data generated from the model. The left panel
    ...
    integrated backward in time for $1.96\Gyr$ (no other changes were
