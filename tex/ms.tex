...
\begin{figure}[t!]
 ...
 \caption{Marginalized, conditional $p(Z|X)$ PDFs. The left panel
   ...
   the left panel; the dash-dotted line is a simple Gaussian
   estimate. The upper branch near $Z \approx 5\kpc$ of the stream is
   chosen for the middle panel. The right panel shows an $X$ close to
   where the stream turns around in $X$; the PDF has multiple peaks in
   this case.}\label{fig:gd1_pdf_xz}
...
integrations over the PDF. In practice, the perform these
marginalizations it is useful to estimate the extent of the PDF to
estimate appropriate intervals for efficient numerical
integration. For this we can use the average stream track and
dispersion around it as estimated in
\sectionname~\ref{sec:track}. This is most easily explained through an
example. To determine $p(Z|X)$, the distribution of Galactocentric $Z$
at a given Galactocentric $X$, we need to calculate
\begin{equation}
...
dispersion at the lower peak as well.
...
and velocity dispersion along the stream.
...
\begin{split}
  ...
  & = \mathrm{Re}\left[\exp(-i\,n_\phi\,\theta^A_\phi-i\,n_Z\,\theta^A_Z)\int \dd \theta_R^A\,\exp(-i\,n_R\,\theta^A_R)\right]\,.
\end{split}
\end{equation}
